...
int main(int argc, char** argv){
	...
	for (time_stamp=0; time_stamp<time_max; time_stamp++) { // for each time stamp
		...
		for(p=0; p<INIT_NO_PARTICLES; p++) { // for all particles
			if(collisions[p]) continue; // If this particle has already collided qith a previous particle, ignore

			/* check for collisions with all next particles*/
			for(pp=p+1; pp<INIT_NO_PARTICLES; pp++){
				if(collisions[pp]) continue; // If the particle we want to colide with has already collided, ignore
